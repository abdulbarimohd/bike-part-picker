...
export function checkChainSpeeds(chain?: Chain, cassette?: Cassette): CompatibilityWarning | null {
  ...
  if (chain.speeds === cassette.speeds) return null;
  return warn('R-DRV-07', 'critical', 'Chain and cassette speed mismatch',
    `${chain.brand} ${chain.name} is a ${chain.speeds}-speed chain against a ${cassette.speeds}-speed cassette. The chain is the wrong width for the cog spacing.`,
    ['chain', 'cassette']);
}
